...
class HotelSearchSpider(scrapy.Spider):
    ...
    def detail_parse(self,response):
        ...
        detail=response.xpath('//*[@id="root"]/div/section[2]/section/section[2]/aside[1]/aside[2]/div/div[3]').get()
        if not detail:
            detail=response.xpath('//*[@id="root"]/div/section[2]/section/section[2]/aside[1]/aside[2]/div/div[2]').get()
...
# 遍历每个dl标签，提取服务类型和相关设施
        for dl in soup.find_all('dl', class_='desc_item clearfix'):
            service_name = dl.find('dt', class_='name').text.strip()
            facilities = [span.text.strip() for span in dl.find_all('span')[1::2]]  # 跳过iconfont的span，只提取服务名称
            services[service_name] = facilities
